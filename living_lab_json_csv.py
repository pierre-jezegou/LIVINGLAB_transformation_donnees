...
# Liste exhaustive des différentes catégories qu'il est possible de trouver dans les évènements du fichier JSON.
# Comme cela, on traite pas à pas les évènements en puisant dans cette liste
fichiers = ["patients", "conditions", "allergies", "visites", "prescriptions", "obs_num", "obs_text", "ref_medications"]


# Exemple de dictionnaire de correspondance pour l'utilisation de l'IA
genres = {
    "female":1,
    "male":0
    }
...
def INITIALISATION() -> None:
    ''' Cette fonction permet de nettoyer l'espace de travail, de créer les fichiers s'ils n'existent pas '''
    print("Nettoyage de l'espace de travail...")
    # On commence par enlever tous les fichiers s'ils existent
    for fichier in fichiers:
        if os.path.exists(PATH + fichier+".csv"):
            os.remove(PATH + fichier+".csv")
    # On crée le dossier qui contiendra les données s'il n'existe pas
    # On regarde s'il n'existe pas
    if not os.path.exists(PATH):
        # On crée le répertoire
        os.mkdir(PATH)
    # On affiche que c'est bien terminé
